	...
	: TForm(Owner)
{
	...
	gTestLog = "C:\\ASMP\\ScreenInfo\\LOG\\"+FormatDateTime("mm-dd_hh-nn-ss", Now())+".txt";
	_mkdir( "C:\\ASMP\\");
	_mkdir( "C:\\ASMP\\ScreenInfo\\" );
	_mkdir( "C:\\ASMP\\ScreenInfo\\LOG\\" );
	bStart = true;
}
void  TfrmMain::SaveLogLine(AnsiString FileName,AnsiString FileLine)
{
	FileLine = (FormatDateTime("[yyyy-mm-dd]hh:nn:ss", Now()))+"\r\n"+FileLine+="\r\n========\r\n";
	FILE * fp;
...
void TfrmMain::CheckDevData()
{
	...
	int ScreenPASS = 0;
	int DisplayPASS= 0;
	TStringList* ScreenList = new TStringList();
	TStringList* DisplayShipList = new TStringList();
	//
	if(TListScreen_Dev->Text != "")
		ScreenList->Text = TListScreen_Dev->Text;
	else ScreenList->Text = "NULL";
	if(TListDisplayLink_Dev->Text != "")
		DisplayShipList->Text = TListDisplayLink_Dev->Text;
	else DisplayShipList->Text = "NULL";
	...
	if(plResult->Caption == "PASS" && !bPASS)
	{
		SaveLogLine(gTestLog,"影像資訊遺失\r\n[Screen]"+ScreenList->Text+"\r\n[DisplayLink]"+DisplayShipList->Text);
	}
	else if(plResult->Caption == "WAIT" && bPASS)
	{
		SaveLogLine(gTestLog,"影像資訊回復\r\n[Screen]"+ScreenList->Text+"\r\n[DisplayLink]"+DisplayShipList->Text);
    }
